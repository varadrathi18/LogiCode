// Initialize lives and points - pass prefix like 'sp-intermediate-' or 'fr-beginner-'
...
// Get lives and points for prefix
...
// Load question and handle clicks for answers
function loadQuestion({answer, next, prefix}) {
  initGame(prefix);
  updateStatusBar(prefix);

  document.querySelectorAll('#play-audio, #spanish-word, #french-word, #english-word, #german-word').forEach(el => {
    el.onclick = e => {
      e.preventDefault();
  ...
  options.forEach(option => {
    option.onclick = () => {
      ...
      if(nextBtn) {
        nextBtn.onclick = () => {
          if(lives === 0) return; // no navigation if no lives and buy/home buttons are shown
